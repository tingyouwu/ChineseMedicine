...
public class UserLoginModel implements IUserLoginContract.IUserLoginModel {
    ...
    @Override
    public void login(final String name, final String psw,final boolean isAutoLogin, final ICallBack<String> callBack) {
        ...
        bu2.login(new SaveListener<UserBmob>() {
            ...
            public void done(UserBmob bmobUser, BmobException e) {
                if(e==null){
                    ...
                    //再次尝试以邮箱登陆
                    bu2.loginByAccount(name, psw, new LogInListener<UserBmob>() {
                        @Override
                        public void done(UserBmob bmobUser, BmobException e) {
                            if(bmobUser!=null){
                                callBack.onSuccess(bmobUser.getObjectId());
